...
// ForbiddenPage renders a 403 page for MAC-filtered requests.
func ForbiddenPage(project, clientIP, mac, serverIP string, _ int) string {
	macStr := mac
	if macStr == "" {
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .badge {
    ...
    background: #fce8e8; color: #c5271f; border: 1px solid #e8b4b4;
  ...
  .row { display: flex; justify-content: space-between; align-items: center; padding: .5rem 0; border-bottom: 1px solid #f0f0f0; font-size: .875rem; }
  ...
  .btn {
    ...
    padding: .5rem 1.125rem; background: #005fb8; color: #fff;
    border-radius: 4px; text-decoration: none; transition: background .12s;
  }
...
</html>`, clientIP, macStr, project)
}

// NotFoundPage renders a 404 page listing all known project domains.
func NotFoundPage(host string, domains []string) string {
	var links strings.Builder
	for _, d := range domains {
		links.WriteString(fmt.Sprintf(`<li><a href="http://%s">%s</a></li>`, d, d))
	}
	return fmt.Sprintf(`<!DOCTYPE html>
<html lang="en">
<head>
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .badge {
    display: inline-block; font-size: .6875rem; font-weight: 700;
    letter-spacing: .1em; text-transform: uppercase;
    background: #f0f0f0; color: #717171; border: 1px solid #e0e0e0;
    padding: 2px 8px; border-radius: 4px; margin-bottom: .75rem;
  }
  ...
  .btn {
    ...
    padding: .5rem 1rem; background: #005fb8; color: #fff;
    border-radius: 4px; text-decoration: none; transition: background .12s;
  }
  .btn:hover { background: #004c99; }
</style>
